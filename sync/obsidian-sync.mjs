...
if(!local&&!token)throw new Error('公网同步需要设置 JOHN_SITES_TOKEN');
let last='';
function md(tasks){
...
async function sync(){
 const headers=token?{'OAI-Sites-Authorization':token}:{};const response=await fetch(new URL('/api/workspace',siteUrl),{headers});if(!response.ok)throw new Error(`同步读取失败：${response.status}`);
