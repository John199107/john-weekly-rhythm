...
const token=process.env.JOHN_SYNC_TOKEN||'';
const bypass=process.env.JOHN_SITES_BYPASS_TOKEN||'';
const vault=process.env.JOHN_OBSIDIAN_VAULT||'D:\\Users\\76114\\Obsidian Vault';
const output=path.join(vault,'00_个人待办工作台_数据.md');
if(!siteUrl)throw new Error('请设置 JOHN_WORKBENCH_URL');
const local=['localhost','127.0.0.1'].includes(new URL(siteUrl).hostname);
if(!local&&!token)throw new Error('公网同步需要设置 JOHN_SYNC_TOKEN');
let last='';
function md(tasks){
...
async function sync(){
 const headers={};if(token)headers['x-john-sync-token']=token;if(bypass)headers['OAI-Sites-Authorization']=`Bearer ${bypass}`;const response=await fetch(new URL('/api/workspace',siteUrl),{headers});if(!response.ok)throw new Error(`同步读取失败：${response.status}`);
