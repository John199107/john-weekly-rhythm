...
async function sync(){
 const headers=token?{'x-john-sync-token':token}:{};const response=await fetch(new URL('/api/workspace',siteUrl),{headers});if(!response.ok)throw new Error(`同步读取失败：${response.status}`);
